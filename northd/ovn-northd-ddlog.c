...
northd_retry_at(struct northd_ctx *ctx, const char *where)
{
    ...
    if (jsonrpc_session_get_n_remotes(ctx->session) > 1) {
...
northd_run(struct northd_ctx *ctx)
{
    ...
    unsigned int seqno = jsonrpc_session_get_seqno(ctx->session);
    ...
    for (int i = 0; i < 50; i++) {
        struct jsonrpc_msg *msg = jsonrpc_session_recv(ctx->session);
        if (!msg) {
            break;
        }
...
static void
set_probe_interval(struct jsonrpc_session *session, int interval)
{
#define DEFAULT_PROBE_INTERVAL_MSEC 5000
    ...
    jsonrpc_session_set_probe_interval(
        session, interval < 0 ? default_interval : interval);
}
...
main(int argc, char *argv[])
{
    ...
    while (!exiting) {
        ...
        if (northd_lock_status(sb_ctx) == HAS_LOCK &&
            nb_ctx->state == S_MONITORING && sb_ctx->state == S_MONITORING) {
