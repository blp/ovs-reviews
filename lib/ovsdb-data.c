...
/* Given 'old' and 'new' with the specified composite 'type', initializes
 * 'added' to the keys or pairs that are in 'new' but not 'old', and 'removed'
 * to the keys or pairs that are in 'old' but not 'new'. */
void
ovsdb_datum_deltas(const struct ovsdb_datum *old,
                   const struct ovsdb_datum *new,
                   const struct ovsdb_type *type,
                   struct ovsdb_datum *added,
                   struct ovsdb_datum *removed)
{
    ovs_assert(ovsdb_type_is_composite(type));
    ...
    for (oi = ni = 0; oi < old->n && ni < new->n; ) {
        const union ovsdb_atom *ok = &old->keys[oi];
        const union ovsdb_atom *ov = &old->values[oi];
        const union ovsdb_atom *nk = &new->keys[ni];
        const union ovsdb_atom *nv = &new->values[ni];

        int c = ovsdb_atom_compare_3way(ok, nk, type->key.type);
        if (c < 0) {
            ovsdb_datum_add_unsafe(removed, ok, ov, type, NULL);
            oi++;
        } else if (c > 0) {
            ovsdb_datum_add_unsafe(added, nk, nv, type, NULL);
            ni++;
        } else {
            if (type->value.type != OVSDB_TYPE_VOID &&
                ovsdb_atom_compare_3way(ov, nv, type->value.type)) {
                if (added != removed) {
                    ovsdb_datum_add_unsafe(removed, ok, ov, type, NULL);
                }
                ovsdb_datum_add_unsafe(added, nk, nv, type, NULL);
                 ...
                 const struct ovsdb_type *type)
{
    if (ovsdb_type_is_composite(type)) {
        ovsdb_datum_deltas(old, new, type, diff, diff);
    } else {
