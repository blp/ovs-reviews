...
raft_fsync_thread(void *raft_)
{
    ...
    for (;;) {
		...
		mc_wrap_ovsdb_log_commit(raft->log,
					 fsync_mc_conn,
					 MC_FSYNC_TID,
					 OVS_SOURCE_LOCATOR);
            if (!error) {
                 ...
                 const struct uuid *eid, struct json *servers)
{
    ...
    struct ovsdb_error *error = mc_wrap_ovsdb_log_write(raft->log, json,
							raft->mc_conn,
							MC_MAIN_TID,
							OVS_SOURCE_LOCATOR);
    json_destroy(json);
		 ...
		 struct jsonrpc *mc_conn)
{
    ...
    struct ovsdb_error *error = mc_wrap_ovsdb_log_write(log, json,
							mc_conn, MC_MAIN_TID,
							OVS_SOURCE_LOCATOR);
    json_destroy(json);
...
raft_parse_log_record__(struct raft *raft, struct ovsdb_parser *p)
{
     ...
     * current term. */
    uint64_t term = raft_parse_uint(p, "term");
...
raft_read_header(struct raft *raft)
{
    /* Read header record. */
    struct json *header;
    struct ovsdb_error *error = mc_wrap_ovsdb_log_read(raft->log, &header,
						       raft->mc_conn,
						       MC_MAIN_TID,
						       OVS_SOURCE_LOCATOR);
    if (error || !header) {
    ...
    if (remote_addresses) {
        ...
        if (snapshot) {
            ...
            raft->snap.term = raft_parse_uint(&p, "prev_term");
            raft->log_start = raft->log_end
                = raft_parse_uint(&p, "prev_index") + 1;
...
raft_read_log(struct raft *raft)
{
    for (;;) {
        struct json *entry;
        struct ovsdb_error *error = mc_wrap_ovsdb_log_read(raft->log, &entry,
							   raft->mc_conn,
							   MC_MAIN_TID,
							   OVS_SOURCE_LOCATOR);
        if (!entry) {
    ...
    bool wait)
{
    ...
    if (rq->requester_conn) {
        ...
        if (!wait) {
            ...
            target->requester_conn = rq->requester_conn;
        }
    } else {
                    ...
                    uint64_t new_log_start, const struct json *new_snapshot)
{
    ...
    for (uint64_t index = new_log_start; index < raft->log_end; index++) {
        ...
        error = mc_wrap_ovsdb_log_write(log, json, raft->mc_conn,
					MC_MAIN_TID, OVS_SOURCE_LOCATOR);
        json_destroy(json);
        if (error) {
            return error;
                   ...
                   uint64_t new_start, const struct json *new_snapshot)
{
    ...
    struct ovsdb_error *error;
    error = mc_wrap_ovsdb_log_replace_start(raft->log, &new_log,
					    raft->mc_conn, MC_MAIN_TID,
					    OVS_SOURCE_LOCATOR);
    if (error) {
        return error;
    ...
    return mc_wrap_ovsdb_log_replace_commit(raft->log, new_log,
					    raft->mc_conn, MC_MAIN_TID,
					    OVS_SOURCE_LOCATOR);
}
    ...
    struct raft *raft, const struct raft_execute_command_reply *rpy)
{
    ...
    if (!cmd) {
        static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(5, 5);
        VLOG_INFO_RL(&rl,
                     SID_FMT" received %s reply from "SID_FMT" for unknown command",
                     SID_ARGS(&raft->sid),
...
{
    log_rpc(raft, rpc, "-->");
    return !mc_wrap_jsonrpc_session_send(
        js, raft_rpc_to_jsonrpc(&raft->cid, &raft->sid, rpc),
        raft->mc_conn, MC_MAIN_TID, OVS_SOURCE_LOCATOR);
}
...
static void
raft_unixctl_kick(struct unixctl_conn *conn, int argc,
                  const char *argv[], void *aux OVS_UNUSED)
{
    bool wait = true;
    if (!strcmp(argv[1], "--no-wait")) {
        wait = false;
        argc--;
        argv++;
    }
...
raft_init(void)
{
    ...
    unixctl_command_register("cluster/leave", "[--force | --no-wait] DB", 1, 2,
                             raft_unixctl_leave, NULL);
    unixctl_command_register("cluster/kick", "[--no-wait] DB SERVER", 2, 3,
