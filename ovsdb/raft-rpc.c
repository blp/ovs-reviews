...
static void
raft_hello_request_uninit(struct raft_hello_request *rq OVS_UNUSED)
{
}

static void
raft_hello_request_clone(struct raft_hello_request *dst OVS_UNUSED,
                         const struct raft_hello_request *src OVS_UNUSED)
{
}
...
{
    dst->last_servers = json_nullable_clone(src->last_servers);
...
void
raft_rpc_clone(union raft_rpc *dst, const union raft_rpc *src)
{
    *dst = *src;
...
raft_rpc_format(const union raft_rpc *rpc, struct ds *s)
{
    ds_put_format(s, SID_FMT" %s", SID_ARGS(&rpc->common.sid),
                  raft_rpc_type_to_string(rpc->type));
    if (rpc->common.comment) {
...
raft_rpc_get_min_sync_index(const union raft_rpc *rpc)
{
    ...
    case RAFT_RPC_INSTALL_SNAPSHOT_REPLY:
        /* XXX This will need to change if install_snapshot_reply becomes able
         * to report an error */
